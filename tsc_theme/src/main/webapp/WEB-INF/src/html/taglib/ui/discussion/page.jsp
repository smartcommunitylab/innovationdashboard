		...
		<aui:form action="<%= formAction %>" method="post" name="<%= formName %>">
											...
											<ul class="lfr-discussion-actions">
												...
												<c:if test="<%= MBDiscussionPermission.contains(permissionChecker, company.getCompanyId(), scopeGroupId, permissionClassName, permissionClassPK, userId, ActionKeys.ADD_DISCUSSION) %>">
													<li class="lfr-discussion-reply-to">
														...
														<a class="btn btn-tsc" href="<%= taglibPostReplyURL %>"><liferay-ui:message key="post-reply" /></a>
													</li>
												</c:if>
			...
			<c:if test="<%= !hideControls && MBDiscussionPermission.contains(permissionChecker, company.getCompanyId(), scopeGroupId, permissionClassName, permissionClassPK, userId, ActionKeys.ADD_DISCUSSION) %>">
				<aui:fieldset cssClass="add-comment" id='<%= randomNamespace + "messageScroll0" %>'>
						...
						<c:otherwise>
							...
							<c:choose>
								...
								<c:otherwise>
									...
									/>
							--%>
									<a class="btn btn-tsc btn-tsc-add-comment" href="<%= taglibPostReplyURL %>"><liferay-ui:message key="add-comment" /></a>
							<%--
								</c:otherwise>
							</c:choose>
							--%>
					...
					<div id="<%= randomNamespace %>postReplyForm<%= i %>" class="post-reply-form" style="display: none;">
						...
						<aui:button-row>
							<aui:button cssClass="btn-comment btn-tsc" id='<%= namespace + randomNamespace + "postReplyButton" + i %>' onClick='<%= randomNamespace + "postReply(" + i + ");" %>' value="<%= postReplyButtonLabel %>" />
							...
							<aui:button cssClass="btn-comment btn-tsc" onClick="<%= taglibCancel %>" type="cancel" />
						</aui:button-row>
					</div>
				</aui:fieldset>
			</c:if>
